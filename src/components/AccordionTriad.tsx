...
const triadItems: TriadItem[] = [
  { id: 'no-politicians', headline: 'NO POLITICIANS', reveal: 'We don\'t hire rulers; we hire public servants. In AreTéCracy, all officials are functionaries, not commanders. There are no career politicians, no dynasties, and no permanent ruling class.\n\nFor high-level offices, citizens are selected from a refined list of highly qualified individuals based on proven merit, skills, and knowledge. This list must be approved by at least 70% of the Ekklesia, and the final magistrate is chosen by Sortition (random lot). This ensures competence while preventing any single political bias from capturing the system. This is Plato\'s Roll Call: the duty to participate or be ruled by your inferiors.' },
  { id: 'no-billionaires', headline: 'BILLIONAIRES CAN\'T BUY VOTES', reveal: 'In the current system, money buys access, and access buys laws. We end this with Isotímia (Equal Honor). We don\'t disparage wealth, but we recognize that the Pleonektai (those who seek more than their share) often use it to extract opportunity from the community.\n\nHere, lobbying is impossible because there are no politicians to lobby. Campaign donations are impossible because there are no campaigns. Your voice is weighted by your citizenship, not your net worth.' },
  { id: 'future-has-vote', headline: 'THE FUTURE GETS A VOTE', reveal: 'Every generation is tempted to live large and hand the bill to people who can\'t vote yet. We stop this practice of "taxation without representation" against our own descendants.\n\nEvery proposed law is stress-tested for its long-term impact. If a policy creates short-term gain at a future cost, the Generational Fairness Fund and Horizon Clauses intervene.' },
  { id: 'watch-watchers', headline: 'WHO WATCHES THE WATCHMEN? YOU', reveal: 'Transparency usually means the government watching you. We flip the cameras. GlaukÓS Mati (The Bright-Eyed Guardian) is a system of "Reverse Surveillance" where transparency flows upward.\n\nEvery action, transaction, and decision made by a public servant is recorded on an immutable public ledger. There are no backroom deals because there are no backrooms.' },
  { id: 'no-51-tyranny', headline: '51% IS NEVER ENOUGH', reveal: 'Modern democracy is often a civil war fought with ballots, where 51% of the people try to crush the other 49%. This creates division, hate, and policy whiplash.\n\nWe believe that any law which half the population hates is, by definition, a bad law. We require Homonoia (Like-mindedness). Major legislation requires broad consensus (often supermajorities of 60-70%) to pass.' },
  { id: 'self-destruct', headline: 'THE SELF-DESTRUCT BUTTON', reveal: 'In most systems, removing a failed government requires a revolution or a civil war. We built a safety valve instead.\n\nAxioktonia (The Worthy Death) is the ultimate guarantee of freedom. If a hypermajority of citizens agree the system has fundamentally failed, they can trigger a peaceful, orderly "uninstall and reinstall" of the government.' },
  { id: 'workshop-economy', headline: 'AN ECONOMY THAT WORKS', reveal: 'You cannot have a free vote if you are a serf in the marketplace. Today, a handful of "Too Big to Fail" monopolies control the survival stack—from food to payments. That isn\'t capitalism; it\'s corporate feudalism.\n\nHephaistia breaks the "Infinity Gauntlet." We stop subsidizing giants and start funding the little guy.' },
  { id: 'civility-default', headline: "WE'LL BURN DOWN THE THEATER", reveal: 'Modern politics is theater: scripted performances, fake outrage, and shouting matches designed to entertain, not govern. We end the spectacle.\n\n**IsoKratÓS** is our AI moderator that replaces the stage with a workshop. It coaches citizens to turn rants into proposals. It enforces the "Steel Man Rule": you cannot criticize an argument until you restate it to your opponent\'s satisfaction.' }
];

export function AccordionTriad({ darkMode = true, onNavigate }: AccordionTriadProps) {
            ...
            return (
              <motion.div
                ...
                className={`border rounded-lg overflow-hidden transition-all duration-300 ${
                  darkMode
                    ? isOpen ? 'bg-gray-800/80 border-cyan-500/50 shadow-lg shadow-cyan-500/20' : 'bg-gray-800/50 border-gray-700 hover:border-gray-600'
                    : isOpen ? 'bg-white border-cyan-600/50 shadow-lg shadow-cyan-600/10' : 'bg-white border-stone-200 hover:border-stone-300'
                }`}
              >
                <button
                  ...
                  className={`w-full px-6 py-5 md:px-8 md:py-6 flex items-center justify-between text-left transition-colors ${
                    darkMode
                      ? isOpen ? 'bg-gray-800/90' : 'bg-gray-800/50 hover:bg-gray-800/70'
                      : isOpen ? 'bg-white' : 'bg-stone-50 hover:bg-stone-100'
                  }`}
                  aria-expanded={isOpen}
                >
                  <h3 className={`text-xl md:text-2xl font-serif font-bold flex-1 ${darkMode ? 'text-amber-200' : 'text-stone-900'}`}>
                    {item.headline}
                  </h3>
                  <motion.div
                    animate={{ rotate: isOpen ? 180 : 0 }}
                    transition={{ duration: 0.3 }}
                    className={`ml-4 flex-shrink-0 ${darkMode ? 'text-cyan-400' : 'text-cyan-600'}`}
                  >
                    <ChevronDown size={24} />
                  </motion.div>
                </button>
                  ...
                  {isOpen && (
                    ...
                    >
                      <div className={`px-6 py-5 md:px-8 md:py-6 border-t ${
                        darkMode ? 'border-gray-700 bg-gray-800/40' : 'border-stone-200 bg-stone-50/50'
                      }`}>
                        <p className={`text-lg md:text-xl leading-relaxed ${darkMode ? 'text-gray-200' : 'text-stone-700'}`} style={{ fontFamily: 'Georgia, serif' }}>
                          {item.reveal}
                        </p>
                      </div>
                    </motion.div>
