...
const features = [
  ...
  { id: 'no-51-tyranny', title: "51% IS NEVER ENOUGH", icon: <Handshake className="w-5 h-5" />, content: "Modern democracy is often a civil war fought with ballots, where 51% of the people try to crush the other 49%. We require Homonoia (Like-mindedness). Major legislation requires broad consensus." },
  { id: 'self-destruct', title: "THE SELF-DESTRUCT BUTTON", icon: <RefreshCw className="w-5 h-5" />, content: "Axioktonia (The Worthy Death) is the ultimate guarantee of freedom. If a hypermajority of citizens agree the system has fundamentally failed, they can trigger a peaceful \"uninstall and reinstall\" of the government." },
  { id: 'workshop-economy', title: "AN ECONOMY THAT WORKS", icon: <Hammer className="w-5 h-5" />, content: "You cannot have a free vote if you are a serf in the marketplace. Hephaistia breaks the \"Infinity Gauntlet.\" We stop subsidizing giants and start funding the little guy." },
  { id: 'civility-default', title: "WE'LL BURN DOWN THE THEATER", icon: <MessageCircle className="w-5 h-5" />, content: "Modern politics is theater: scripted performances, fake outrage, and shouting matches designed to entertain, not govern. **IsoKratÓS** is our AI moderator that replaces the stage with a workshop." },
];
    ...
    <section id="eight-ways" className={`py-16 md:py-24 ${darkMode ? 'bg-gray-900' : 'bg-stone-50'}`}>
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-6 md:mb-12">
          ...
          <p className={`text-lg md:text-xl max-w-2xl mx-auto ${darkMode ? 'text-gray-300' : 'text-stone-700'}`}>Click a principle to learn more.</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-12">
          {features.map((feature) => (
            <button key={feature.id} type="button" onClick={(e) => handleCardClick(e, feature)}
            ...
            >
              <div className="flex items-center gap-3 flex-1 min-w-0">
                <div className={`p-2 rounded-full flex-shrink-0 ${darkMode ? 'bg-gray-950 border border-gray-800 group-hover:border-cyan-500/50 text-cyan-400' : 'bg-stone-100 border border-stone-300 text-amber-600'}`}>
                  {feature.icon}
                </div>
                <h3 className={`font-serif text-xl md:text-2xl leading-tight flex-1 ${darkMode ? 'text-gray-200 group-hover:text-white' : 'text-stone-800 group-hover:text-stone-900'}`}>
                  {feature.title}
                </h3>
              </div>
            </button>
        ...
        <div className="flex flex-col sm:flex-row justify-center gap-4 items-stretch">
          <button onClick={() => onNavigate?.('pillars')}
          ...
          >SEE THE BLUEPRINT <ChevronRight size={20} /></button>
          <a href="https://www.patreon.com/aretecracy" target="_blank" rel="noopener noreferrer"
