...
// Flexible tile configuration - can be rearranged or extended (order = display order; id links to card)
const triadItems: TriadItem[] = [
  {
    id: 'no-politicians',
    headline: 'NO POLITICIANS. NOT ONE.',
  ...
  {
    id: 'no-billionaires',
    headline: 'BILLIONAIRES CAN\'T BUY VOTES. OR LAWS.',
  ...
  {
    id: 'future-has-vote',
    headline: 'THE FUTURE GETS TO VOTE, TOO.',
  ...
  {
    id: 'workshop-economy',
    headline: 'AN ECONOMY THAT REMEMBERS EVERYONE.',
  ...
  {
    id: 'watch-watchers',
    headline: 'WHO WATCHES THE WATCHMEN? YOU DO.',
  ...
  {
    id: 'no-51-tyranny',
    headline: 'THE FIVE-ONE IS A COP-OUT.',
  ...
  {
    id: 'civility-default',
    headline: 'THE THEATER GETS ITS FINAL CURTAIN CALL.',
  ...
  {
    id: 'self-destruct',
    headline: 'IF ALL ELSE FAILS, SELF DESTRUCT.',
...
export function FeatureTriadOverlay({ darkMode = true, onNavigate, onNavigateToPillar }: FeatureTriadOverlayProps) {
  ...
  const handleTileClick = (item: TriadItem, e: React.MouseEvent) => {
    // Capture click position for bloom animation
    ...
    <>
      <section id="eight-ways" className="scroll-mt-20 md:scroll-mt-24 pt-8 md:pt-12 pb-16 md:pb-24 bg-transparent">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {/* Kit-Kat Tiles - Flexible grid layout: stacked mobile, 4x2 grid desktop */}
              ...
              >
                ...
                <div className={`absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 ${
                  darkMode
                    ...
                    : 'bg-gradient-to-r from-cyan-400/15 via-cyan-500/25 to-cyan-400/15'
                } blur-xl -z-10`} />
                
                {/* Blue glow border effect on hover */}
                <div className={`absolute -inset-0.5 opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-lg ${
                  darkMode
                    ? 'bg-gradient-to-r from-cyan-500/60 via-cyan-400/80 to-cyan-500/60'
            ...
            <a
              ...
              className={`w-full sm:w-auto px-8 py-4 border-2 font-bold rounded-sm transition-all flex items-center justify-center gap-2 ${
                darkMode
                  ...
                  : 'border-[#8B6914] text-[#8B6914] hover:bg-[#D4AF37]/20'
              }`}
            >
              JOIN US
            </a>
      ...
      {/* DetailCard Overlay - Using merged format with triad mode */}
      <AnimatePresence>
        {selectedItem && cardOrigin && (
          <DetailCard
            element={{
              id: selectedItem.id,
              name: selectedItem.headline,
              type: 'concept'
            }}
