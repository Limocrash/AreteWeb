...
export function GlossaryPage({ darkMode, onNavigateToPillar }: GlossaryPageProps) {
  ...
  useEffect(() => {
    ...
    const handleStorageChange = (e: StorageEvent) => {
      if (e.key === 'aretecracy-temple-progress') loadCompletedTerms();
    };
  ...
  // Explicit type guard so TypeScript narrows Set<string> correctly
        ...
        <div className="text-center mb-12">
          ...
          >
            <div className="relative w-5 h-5 shrink-0 flex items-center justify-center">
              <div
                className={`absolute inset-0 z-0 rounded-full blur ${darkMode ? 'bg-cyan-400/40' : 'bg-cyan-600/30'}`}
            ...
            {searchQuery && (
              <button
                ...
                className={`absolute right-4 top-1/2 -translate-y-1/2 p-1 rounded-full transition-colors ${
                  darkMode ? 'text-amber-400 hover:bg-stone-700' : 'text-stone-500 hover:bg-stone-100'
                }`}
              >
                <X className="w-5 h-5" />
              </button>
            ...
            {filterCategory && (
              <button
                ...
                className={`px-3 py-1 rounded-full text-xs transition-all ${
                  darkMode ? 'bg-stone-600 text-amber-200 hover:bg-stone-500' : 'bg-stone-300 text-stone-700 hover:bg-stone-400'
                }`}
              >
                Clear Filters
              </button>
                ...
                {isCompleted && (
                  <div className="absolute top-3 right-3 group z-10" title="You've read this term" onClick={(e) => e.stopPropagation()}>
                    <div className="relative w-[50px] h-[50px]">
                      <div
                        className={`absolute inset-0 z-0 rounded-full ${darkMode ? 'bg-cyan-400/40' : 'bg-cyan-600/30'} blur-md`}
                    ...
                    <div className="absolute right-0 top-6 opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none z-20">
                      <div className={`px-2 py-1 rounded text-xs whitespace-nowrap ${
                        darkMode ? 'bg-stone-800 text-cyan-300 border border-cyan-500/50' : 'bg-white text-cyan-700 border border-cyan-300 shadow-lg'
                      }`} style={{ fontFamily: 'Georgia, serif' }}>Read</div>
                    </div>
                  </div>
                )}
                <div className="mb-3">
                  <h3 className={`text-xl mb-1 transition-colors duration-300 ${darkMode ? 'text-amber-100' : 'text-stone-900'}`} style={{ fontFamily: 'Georgia, serif' }}>
                    {term.name}
                  </h3>
                  {term.greekName && (
                    <p className={`text-sm transition-colors duration-300 ${darkMode ? 'text-cyan-300' : 'text-cyan-600'}`}>{term.greekName}</p>
                  )}
                </div>
                <p className={`text-sm mb-3 transition-colors duration-300 ${darkMode ? 'text-amber-200/80' : 'text-stone-700'}`} style={{ fontFamily: 'Georgia, serif' }}>
                  {term.blurb}
                </p>
                <div className="flex gap-2 flex-wrap">
                  {term.category && (
                    <span className={`text-xs px-2 py-1 rounded-full ${darkMode ? 'bg-stone-700 text-cyan-300' : 'bg-stone-200 text-stone-700'}`}>
                      {term.category}
                    </span>
                  )}
                  {term.hasFullPage && (
                    <span className={`text-xs px-2 py-1 rounded-full flex items-center gap-1 border ${
                      darkMode ? 'bg-[#020617] border-gray-700 text-stone-50' : 'bg-white/90 border-amber-400 text-slate-900'
                    }`}>
